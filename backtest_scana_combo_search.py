...
DATA_DIR = "data"
OUTPUT_REPORT = os.path.join(DATA_DIR, "scana_combo_winrate_leaderboard.json")
MAX_PE = 35.0
...
def simulate_combo(data_cache, base_w, vol_m, pct_m, min_dots, max_risk, target_pct, be_trigger):
    trades = []
...
def main():
    print("🚀 Running Exhaustive Combinatorial Optimization across Top Presets...")
    ...
    # High-Performing Parameter Ranges from OFAT Audit
    grid_base_w = [30, 35, 40]
    grid_vol_m = [1.30, 1.40, 1.50]
    grid_pct_m = [1.30, 1.40]
    grid_min_dots = [3, 4, 5]
    grid_max_risk = [5.0, 7.0, 8.0]
    grid_target = [15.0, 18.0]
    grid_be = [10.0, 13.0, 15.0]
    ...
    print(f"⚙️ Evaluating {total} high-probability combinations...")

    results = []
    start_t = time.time()

    for idx, (bw, vm, pm, dots, sl, tgt, be) in enumerate(all_combos):
        stats = simulate_combo(data_cache, bw, vm, pm, dots, sl, tgt, be)
        if stats:
            results.append({
                "Params": {
                    "Base": f"{bw}d",
                    "Vol Multiplier": f"{vm}x",
                    "Deliv Multiplier": f"{pm}x",
                    "Cluster Dots": f"{dots} dots",
                    "Max SL": f"{sl}%",
                    "Partial Book": f"+{tgt}%",
                    "BE Shift": f"+{be}%"
                },
        ...
        if (idx + 1) % 150 == 0:
            print(f"⏳ Evaluated {idx + 1}/{total} combinations ({round(time.time() - start_t, 1)}s elapsed)...")

    # Sort strictly by Highest Win Rate first, then Profit Factor
    results.sort(key=lambda x: (x["Metrics"]["win_rate_val"], x["Metrics"]["pf"]), reverse=True)
    top_20 = results[:20]
    ...
    payload = {
        ...
        "Top 20 Win Rate Presets": top_20
    }
    ...
    print(f"\n🎉 Done! Top combination achieved {top_20[0]['Metrics']['win_rate']} Win Rate (PF {top_20[0]['Metrics']['pf']}).")
