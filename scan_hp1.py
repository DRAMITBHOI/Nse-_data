...
# RANK #3 EXACT PARAMETER DEFINITION
BASE_W = 40             # 40-day base window
VOL_M = 1.50            # 1.5x 20-day delivery volume SMA
PCT_M = 1.40            # 1.4x 50-day delivery percentage SMA
MIN_DOTS = 5            # Exactly >= 5 qualified institutional accumulation dots
MAX_RISK = 8.0          # Stop loss cap <= 8.0%
TARGET_PCT = 15.0       # Book 50% at +15.0%
BE_TRIGGER = 15.0       # Shift stop loss to breakeven at +15.0%
MAX_PE = 35.0
...
def scan_hp1():
    print(f"🚀 Running Scan HP1 Production Scanner (Rank #3 Configuration)...")
    print(f"⚙️ Settings: Base={BASE_W}d | VolMult={VOL_M}x | DelivMult={PCT_M}x | MinDots={MIN_DOTS} | MaxRisk={MAX_RISK}% | Target={TARGET_PCT}%")
    ...
    for f_name in stock_files:
        ...
        # Track institutional accumulation dots
        ...
        # -----------------------------------------------------------------
        # 1. HISTORICAL TRADES SIMULATION (For Visualizer & Markers)
        # -----------------------------------------------------------------
        symbol_trades = []
        cooldown = 0
        ...
        # -----------------------------------------------------------------
        # 2. LIVE TODAY SCREENER EVALUATION (Latest Bar = N - 1)
        # -----------------------------------------------------------------
        curr_idx = N - 1
        ...
        if num_dots_curr >= MIN_DOTS and vol_sma9[curr_idx] >= MIN_AVG_VOLUME_9D:
            ...
            if 0 < risk_pct <= MAX_RISK:
                ...
                if is_triggered or (0.0 <= dist_pct <= 2.5):
                    status = "🟢 BREAKOUT TRIGGERED" if is_triggered else "🟡 COILING AT CEILING"
                    live_candidates.append({
                        ...
                        "Demat Dots": f"{num_dots_curr} dots (≥1.5x Vol)",
                        "Distance to Breakout": "0.0%" if is_triggered else f"{dist_pct}%",
                        "P/E": f"{pe_float:.1f}" if pe_float else "N/A"
                    })

    live_candidates.sort(key=lambda x: (x["Status"].startswith("🟢"), -float(x["Risk %"].replace("%", ""))), reverse=True)

    payload = {
        "Scan Timestamp": time.strftime("%Y-%m-%d %H:%M:%S IST"),
        "Rank Config": "Rank #3: Base 40d | 5 Dots (1.5x Vol, 1.4x Deliv) | Max SL 8% | Book +15% | BE +15%",
    ...
    print(f"✅ HP1 Scan Complete! Found {len(live_candidates)} active setups. Saved to {OUTPUT_FILE}.")
