...
DATA_DIR = "data"
OUTPUT_FILE = os.path.join(DATA_DIR, "scan_hp3_results.json")

# RANK #16 ADAPTED (OUTSIDE NIFTY 750) PARAMETERS
BASE_W = 60             # Extended 60-day base window
...
MAX_RISK = 15.0         # Extended stop loss cap <= 15.0%
TARGET_PCT = 15.0       # Book 50% at +15.0%
BE_TRIGGER = 10.0       # Shift stop loss to breakeven at +10.0%
MAX_PE = 35.0
MIN_AVG_VOLUME_9D = 25000  # Reasonable floor to filter dead penny illiquidity
...
def scan_hp3():
    print("🚀 Running Scan HP3 Production Scanner (Rank #16 Outside Nifty 750)...")
    ...
    print(f"📦 Loaded {len(nifty_750_set)} Nifty 750 symbols to exclude.")
    ...
    # STRICT FILTER: Keep ONLY stocks outside Nifty 750
    stock_files = [f for f in all_stock_files if f.replace(".json", "").strip().upper() not in nifty_750_set]
    ...
    for f_name in stock_files:
        ...
        # Institutional accumulation dots (1.5x Vol, 1.3x Deliv%)
    ...
    payload = {
        ...
        "Rank Config": "Rank #16 (Outside Nifty 750): Base 60d | ≥4 Dots (1.5x Vol, 1.3x Deliv) | Max SL 15% | Book +15% | BE +10%",
    ...
    print(f"✅ HP3 Scan Complete! Found {len(live_candidates)} candidates outside Nifty 750. Saved to {OUTPUT_FILE}.")
